...
/* ---------- licencia Pro (Gumroad, sin backend) ---------- */
const GUMROAD_PRODUCT_ID = "PENDIENTE_CREAR_PRODUCTO_EN_GUMROAD"; // Gumroad -> tu producto -> "Product ID" (pestana Content o Share), pegalo aqui
const GUMROAD_PRODUCT_URL = "https://gumroad.com/l/PENDIENTE"; // URL publica del producto, para el boton "Comprar en Gumroad"
const LICENSE_STORE = "fin:license";
...
function setLicenseState(state){
  try{ localStorage.setItem(LICENSE_STORE, JSON.stringify(state)); }catch(e){}
}
function clearLicenseState(){
  try{ localStorage.removeItem(LICENSE_STORE); }catch(e){}
}
function isPro(){
  const s = getLicenseState();
  return !!(s && s.valid === true && s.key);
}
// Unica funcion de lib.js con I/O (fetch): vive aqui para que toda la logica de
// licencia quede junta y sea facil de encontrar/editar, aunque no sea pura.
async function verifyLicenseWithGumroad(key, productId){
  productId = productId || GUMROAD_PRODUCT_ID;
  key = String(key || "").trim();
  if(!key) return {success:false, message:"Introduce una clave de licencia."};
  if(!productId || productId === "PENDIENTE_CREAR_PRODUCTO_EN_GUMROAD"){
    return {success:false, message:"Falta configurar GUMROAD_PRODUCT_ID en lib.js con el ID real del producto de Gumroad."};
  }
  try{
    ...
    if(data && data.success){
      setLicenseState({key, valid:true, checkedAt:Date.now(), uses:data.uses});
      return {success:true};
    }
    return {success:false, message:(data && data.message) || "Clave no valida para este producto."};
  }catch(e){
    return {success:false, message:"No se pudo verificar (revisa tu conexion) — " + String((e && e.message) || e)};
  }
}
