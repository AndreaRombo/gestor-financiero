...
2. `driveDiscoverTabs()` intenta leer todas las pestañas de la hoja haciendo `fetch` a la página de edición pública y buscando un patrón en su HTML interno — técnica no oficial, puede romperse si Google cambia el marcado. Si falla (o CORS lo bloquea), cae a importar solo la pestaña del `gid` del enlace pegado. **Google sirve dos formatos de página distintos según el `User-Agent`:** con un navegador de escritorio responde `/edit` con el bloque de datos que busca el primer patrón; con un `User-Agent` de móvil responde con un `302` a `/edit#gid=…` → `/htmlview` (vista simplificada), que no contiene ese bloque pero sí trae los mismos datos en un formato de texto distinto (`items.push({name: "...", pageUrl: "...", gid: "..."` ). `driveDiscoverTabs()` prueba primero el patrón de escritorio y, si no encuentra nada, prueba el de `/htmlview` — así el descubrimiento funciona también desde el móvil en vez de caer siempre al único tab genérico "Google Sheets". Confirmado con `curl` contra una hoja real: sin este segundo patrón, un `User-Agent` de móvil encontraba 0 pestañas; con él, las mismas 15.
...
**Fix: aviso claro cuando el secreto de escritura no coincide entre dispositivos** (`reportWriteResult()`, `writeStep1` del modal `#writeOverlay`): `saveWriteUrl()` genera el código secreto **localmente** la primera vez que ese dispositivo guarda una URL de Apps Script sin tener ya uno (`fin:writesecret` vacío) — si en un segundo dispositivo (el móvil) se pega la misma URL a mano en vez de abrir el "Enlace para configurar otro dispositivo" (que sí lleva el secreto original dentro, ver más abajo), ese dispositivo se inventa un secreto propio que el Apps Script desplegado no reconoce, y todo intento de escritura falla con `{"error":"unauthorized"}` — confirmado contra un despliegue real con `curl` (POST con secreto incorrecto ⇒ `200 OK` con ese JSON, CORS habilitado; el propio Apps Script funcionaba bien, el problema era solo el secreto). Antes, `reportWriteResult()` mostraba el mismo mensaje genérico para cualquier fallo, sin pista de la causa real. Ahora: (1) `reportWriteResult()` detecta `reason==="unauthorized"` y explica la causa concreta con el paso a seguir; (2) `writeStep1` del modal advierte de antemano, antes de guardar ninguna URL, de que no hay que volver a pegarla a mano si ya está configurada en otro dispositivo.
...
**Fix: causa raíz del fallo en móvil (`/htmlview` en vez de `/edit`)**: el punto anterior amortiguaba el síntoma (caía a la caché de pestañas conocidas, o a un enlace de configuración generado en otro dispositivo) pero no arreglaba por qué `driveDiscoverTabs()` fallaba en móvil. La causa: con `User-Agent` de móvil, Google redirige la petición a `/htmlview` en vez de servir `/edit`, y esa página no tiene el bloque de datos que buscaba el único patrón existente. Con el segundo patrón (ver arriba, sección "Integración con Google Sheets") el descubrimiento funciona directamente en móvil, así que la red de seguridad de este punto pasa a ser solo eso — una red de seguridad para el caso (raro) de que ambos patrones fallen, no el camino habitual para que un móvil vea todas las pestañas.
